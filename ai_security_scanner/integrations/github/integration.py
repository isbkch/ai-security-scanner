...
class GitHubIntegration:
    ...
    def __init__(self, config: Config):
        ...
        if not self.github_token:
            raise ValueError(f"GitHub token not found in environment variable: {config.github.token_env}")
            ...
            try:
                ...
                if self.analyzer and scan_result.vulnerabilities:
                    ...
                    context = {
                        ...
                        'language': self.scanner.get_supported_languages()[0] if self.scanner.get_supported_languages() else 'unknown'
                    }
    ...
    def _get_check_conclusion(self, scan_result: ScanResult) -> str:
        ...
        critical_count = sum(1 for v in scan_result.vulnerabilities if v.severity == Severity.CRITICAL)
    ...
    def _create_check_details(self, scan_result: ScanResult) -> str:
        ...
        for file_path, vulns in vuln_by_file.items():
            ...
            for vuln in vulns:
                ...
                details.append(f"- {severity_emoji.get(vuln.severity.value, '⚪')} **{vuln.vulnerability_type}** (Line {vuln.location.line_number})")
