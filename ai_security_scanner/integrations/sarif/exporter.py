...
class SARIFExporter:
    ...
    def _create_run(self, scan_result: ScanResult) -> Dict[str, Any]:
        ...
        run = {
            ...
            "results": [
                self._create_result(vuln, scan_result)
        ...
        if scan_result.repository_url or scan_result.commit_hash:
            run["versionControlProvenance"] = [
                self._create_version_control_details(scan_result)
            ]
    ...
    def _create_result(self, vulnerability: VulnerabilityResult, scan_result: ScanResult) -> Dict[str, Any]:
            ...
            "locations": [
                {
                    "physicalLocation": {
                        ...
                        "region": {
                            ...
                            "endLine": vulnerability.location.end_line_number or vulnerability.location.line_number,
                            "endColumn": vulnerability.location.end_column_number or vulnerability.location.column_number,
                            "snippet": {
        ...
        if vulnerability.ai_explanation:
            result["properties"]["ai_analysis"] = vulnerability.ai_explanation
        
        if vulnerability.false_positive_likelihood is not None:
            result["properties"]["false_positive_likelihood"] = vulnerability.false_positive_likelihood
