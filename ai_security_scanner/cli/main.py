...
from ai_security_scanner.core.config import load_config, Config
...
@click.option('--output', '-o', type=click.Choice(['json', 'sarif', 'table']), default='table', help='Output format')
...
@click.option('--language', '-l', multiple=True, help='Languages to scan (can be specified multiple times)')
...
def output_table(result: 'ScanResult') -> None:
    ...
    sorted_vulnerabilities = sorted(
        result.vulnerabilities,
        key=lambda v: severity_order.get(v.severity.value, 4)
    )
