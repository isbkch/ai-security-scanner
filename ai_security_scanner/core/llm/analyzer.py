...
from ai_security_scanner.core.llm.providers import create_llm_provider, LLMProvider
...
class VulnerabilityAnalyzer:
    ...
    async def analyze_vulnerabilities(
        ...
        for i in range(0, len(vulnerabilities), batch_size):
            ...
            tasks = [
                self._analyze_single_vulnerability(vuln, source_code, context)
                for vuln in batch
            ]
    ...
    async def _analyze_single_vulnerability(
        ...
        try:
            ...
            code_snippet = self._extract_code_snippet(
                source_code, 
                vulnerability.location.line_number,
                context_lines=10
            )
            ...
            if self.config.scanner.false_positive_reduction:
                if enhanced_vulnerability.false_positive_likelihood and enhanced_vulnerability.false_positive_likelihood > 0.8:
                    logger.info(f"Filtering out likely false positive: {vulnerability.vulnerability_type} at {vulnerability.location.file_path}:{vulnerability.location.line_number}")
                    return None
    ...
    def _extract_code_snippet(self, source_code: str, line_number: int, context_lines: int = 5) -> str:
    ...
    def _enhance_vulnerability_with_analysis(
        ...
        enhanced.false_positive_likelihood = false_positive_check.get('false_positive_likelihood', 0.5)
    ...
    def find_similar_vulnerabilities(
        ...
        try:
            ...
            other_codes = [vuln.code_snippet for vuln in all_vulnerabilities if vuln.id != target_vulnerability.id]
