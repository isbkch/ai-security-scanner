...
from ai_security_scanner.core.models import (
    ScanResult, VulnerabilityResult, AnalysisContext, Location
)
...
class SecurityScanner:
    ...
    def scan_file(self, file_path: str) -> List[VulnerabilityResult]:
        ...
        try:
            ...
            if language not in self.config.scanner.languages:
                logger.debug(f"Language {language} not in supported languages, skipping: {file_path}")
                return []
            ...
            if pattern.is_supported_language(context.language):
                try:
                    ...
                    filtered_results = [
                        ...
                        if self._get_confidence_score(result.confidence) >= self.config.scanner.confidence_threshold
                    ]
    ...
    def scan_code(self, code: str, language: str, file_path: str = "inline") -> List[VulnerabilityResult]:
