    ...
    def validate(self) -> None:
        ...
        if self.scanner.enable_ai_analysis:
            if not self.get_api_key(self.llm.api_key_env):
                errors.append(f"LLM API key not found in environment variable: {self.llm.api_key_env}")
        ...
        if not self.get_api_key(self.github.token_env):
            errors.append(f"GitHub token not found in environment variable: {self.github.token_env}")
