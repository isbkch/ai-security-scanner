    ...
    def get_model(self, model_name: str, device: torch.device) -> Tuple[AutoTokenizer, AutoModel]:
        ...
        with self._lock:
            if cache_key in self._models:
                ...
                logger.debug(
                    f"Reusing cached model {model_name}, refs: {self._model_refs[cache_key]}"
                )
...
class CodeBERTEmbedder:
    ...
    def _ensure_model_loaded(self) -> None:
        ...
        try:
            self.tokenizer, self.model = self._model_registry.get_model(
        ...
        try:
            ...
            for i in range(0, len(code_snippets), batch_size):
                ...
                for snippet, snippet_vector in batch_embeddings:
                    try:
                        similarity_score = self._cosine_similarity_vectors(
                            target_vector, snippet_vector
                        )
    ...
    def _estimate_code_quality(self, code: str) -> float:
        ...
        doc_lines = [
            line for line in lines if '"""' in line or "'''" in line or line.strip().startswith("#")
        ]
    ...
    def _calculate_pattern_diversity(self, embedding_vector: np.ndarray) -> float:
        ...
        try:
            ...
            chunks = [
                embedding_vector[i : i + chunk_size]
    ...
    def _analyze_security_patterns(
        self, code: str, language: str, embedding_vector: np.ndarray
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_risk_patterns": [
                p for p in detected_patterns if p in ["eval(", "exec(", "os.system", "Runtime.exec"]
            ],
            "medium_risk_patterns": [
                p for p in detected_patterns if p in ["subprocess", "innerHTML", "document.write"]
            ],
        }
    ...
    def _calculate_overall_risk(self, structural: Dict, semantic: Dict, security: Dict) -> float:
        ...
        try:
            ...
            security_risk = (
                security.get("risk_score", 0) * 0.8
                + security.get("embedding_security_score", 0) * 0.2
            )

            # Combine with weights: security most important, then structure, then semantic
            overall_risk = security_risk * 0.5 + structure_risk * 0.3 + semantic_risk * 0.2
    ...
    def _analyze_embedding_security_signals(self, embedding_vector: np.ndarray) -> float:
        ...
        try:
            ...
            risk_signal = min(
                1.0, (max_magnitude * 0.3 + mean_magnitude * 0.3 + variance * 0.4) / 3.0
            )
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_usage_percent": (
                int((cache_size / self.cache_size_limit) * 100) if self.cache_size_limit > 0 else 0
            ),
